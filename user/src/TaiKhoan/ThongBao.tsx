...
const ThongBao: React.FC = () => {
  ...
  useEffect(() => {
    if (!tai_khoan_id) return;

    axios
      .get(`http://localhost:5000/api/thong-bao?tai_khoan_id=${tai_khoan_id}`)
      .then((res) => {
  ...
  const xoaTatCaDaDoc = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/thong-bao/da-doc?tai_khoan_id=${tai_khoan_id}`);
                  ...
                  {thongBaoList.map((tb) => (
                    ...
                    >
                      ...
                      <small>
                        ...
                        ></i>
                        {new Date(tb.thoi_gian).toLocaleString()}
                      </small>
