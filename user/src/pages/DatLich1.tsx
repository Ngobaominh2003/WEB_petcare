...
const DatLich1: React.FC<Partial<DatLich1Props>> = ({
  isOpen = false,
  dichVu = null,
  onClose = () => {},
}) => {
  ...
  const fetchAvailableTimes = async (ngayDat: string) => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5000/api/datlich/thoi_gian_trong",
        {
  ...
  const gioOptions = availableTimes.map((time) => ({
    value: time,
    label: time,
  }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/datlich",
        requestData
      );
    ...
    >
      ...
      {loading ? (
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            Dịch vụ:{" "}
            <strong>{dichVu?.ten_dich_vu || "Chưa chọn dịch vụ"}</strong>
          </div>
